...
osa_fname_cone = [dir_, '20220524175045_osa_drop_1100mM_45asagar.txt'];
osa_fname_cone = [dir_, '20221012165622_osa_drop_1100mM_45agar_inDI.txt'];
osa_fname_cone = [dir_, '20221012104514_osa_drop_1100mM_inDI2.txt'];
osa_fname_cone = [dir_, '20230412183400_osa_drop_1100mM_45asagar.txt'];
...
for i = 1+3*120%1:length(time_wind)
...
for ii =1:length(osb_add)
    ...
    for jj = 1:16
        ...
        if osb_add(ii)>4
            temp(jj) = OP_map(ii,jj).f(data_h22(jj,tt-0));  %ppm reading  -33
        else
...
figure()
subplot(2,2,1); osa2hex(drop_ppm_wa_init);colorbar(); caxis([1 250])
subplot(2,2,2); osa2hex(drop_ppm_wa_3min);colorbar(); caxis([1 150])
subplot(2,2,3); osa2hex(drop_ppm_wo_init);colorbar(); caxis([1 250])
subplot(2,2,4); osa2hex(drop_ppm_wo_3min);colorbar(); caxis([1 150])
