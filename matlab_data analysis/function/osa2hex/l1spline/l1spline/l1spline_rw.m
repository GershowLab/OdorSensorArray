function [z, error] = l1spline_rw(y, s, l, nBreg, nInner, epsilon)
...
%
% modified by Rui Wu for GPU compatibility, Jan 2021
...
z = InitialGuess(y, W);
b = gpuArray(zeros(size(y)));
d = gpuArray(zeros(size(y)));

error = gpuArray(zeros(nBreg, 1));
...
if all(W)
    while nit <= nBreg && e >= epsilon
        ...
        zPrev = z(:);
        for inner = 1:nInner
            z = idctn_rw(Gamma .* dctn_rw(d + y - b));
            d = shrink(z-y+b, 1/l);
        end
        b = b + (z - y - d);
    ...
    while nit <= nBreg && e >= epsilon
        ...
        zPrev = z(W);
        for inner = 1:nInner
            z = idctn_rw(Gamma .* dctn_rw(W .* (d + y - b - z) + z));
            u = z-y+b;
            d(W) = shrink(u(W), 1/l);
        end
