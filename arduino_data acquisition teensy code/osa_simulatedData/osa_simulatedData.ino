/*
 * OSA debug code, silent, generates fake sensor data, doesn't require OSBs to be powered or connected
 * - no verbose output
...
void detectOSB() {
  ...
  */
  // fake response: all OSBs valid (4 total if bus 1, 3 total if bus 2)
  uint8_t osbn;
  if(bus_num==1) {
    osbn = OSB_N;
  } else {
...
bool testAllOSB() {
  ...
  */
  // fake response: always successful
  Serial.write(1);
  return true;
}
    ...
    if((f_hs>0)&(t1_hs>=t0_hs+dt_hs)) {
      ...
      */
      ...
      for(int i=0; i<OSB_N; i++) {
        ...
        */
        // if HS=on, proceed to (pretend to) do measurement
        if(hs_on[i]==true) {
          readHS(i);
        }
      }
...
void minResetOS_allOSB() {
  // fake behavior: basically do nothing while honoring wait times
...
// none of these methods are required for fake data generation
...
// only readOS_raw() and readHS() are required for fake data generation
...
//    openChannel(ma,mb,CH[i]);
    ...
    */
    ...
    data_buf[0] = millis();
    data_buf[6] = uint16_t(random(0,65536));
    data_buf[7] = uint16_t(random(0,65536));
    // get serial ID
    /*
    if(os.setSerialID()>0) {
      data_buf[8] = 0;
...
//    openChannel(ma,mb,CH[i]);
    ...
    */
    // generate fake measurements
    data_buf[0] = millis();
    data_buf[6] = uint32_t(random(0,100));
    data_buf[7] = uint32_t(random(0,100));
    // get serial ID
    /*
